#include "../parser/TypeScriptLexerBase.h"
...
bool TypeScriptLexerBase::getStrictDefault() { return useStrictDefault; }
...
bool TypeScriptLexerBase::IsStrictMode() { return useStrictCurrent; }
...
void TypeScriptLexerBase::ProcessOpenBrace() {
  bracesDepth++;
  useStrictCurrent = scopeStrictModes.size() > 0 && scopeStrictModes.top()
                         ? true
                         : useStrictDefault;
...
bool TypeScriptLexerBase::IsInTemplateString() {
  return templateDepth > 0 && bracesDepth == 0;
}
