 ...
 */
void FileUtil::writeToFile(const std::string &fileName,
...
ExecResult FileUtil::exec(const std::string &cmd) {
  ...
  if (!pipe)  // GCOV_EXCL_LINE
    throw CompilerError(IO_ERROR,
                        "Failed to execute command: " + cmd);  // GCOV_EXCL_LINE
  char buffer[128];
  std::string result;
  while (!feof(pipe)) {
...
#ifdef OS_UNIX
  if (FileUtil::fileExists("/usr/lib/spice/std")) return "/usr/lib/spice/std/";
#endif
  if (std::getenv("SPICE_STD_DIR") &&
      FileUtil::dirExists(std::string(std::getenv("SPICE_STD_DIR")))) {
